...
#include "executables/ROOTPlotHelper.hpp"
...
int main(int argc, char **argv){
  ...
  std::shared_ptr<JakeReader> myPHSPReader(
            new JakeReader("executables/Examples/SliceFit/JPSIPSMC.ACC.root", "kin"));
 ...
 // std::cout << "Fixing 5 of 7 parameters " << std::endl;
  ...
  std::string fitres="executables/Examples/SliceFit/FitResultsAllSlicesUB.txt", fitresroot="executables/Examples/SliceFit/FitResultJAKESLICEUB.root";
  if(argc>1){
  ...
  //std::vector<std::shared_ptr<TH1D> > histData, histModel, histModelCl;
  for(int i=0+(nBins/8.); i<nBins-(nBins/8.)-1; i++){
  //  {unsigned int i=50;
