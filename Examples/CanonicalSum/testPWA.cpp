...
int main(int argc, char **argv) {
  ...
  config.add_options()("disableFileLog",
                       ...
                       "write log file? 0/1");
  config.add_options()(
      "nEvents", po::value<int>(&numEvents)->default_value(100),
...
//  bool calculateInterference;
  ...
  config_fit.add_options()(
      ...
      "Precision form MC integration/cross section normalization. Number of events of phase space MC sample.");
  config_fit.add_options()(
      "ampMcPrecision",
      po::value<unsigned int>(&ampMcPrecision)->default_value(10000),
      "Precision for amplitude/breit-wigner normalization. Number of events of toy phase sample.");
  config_fit.add_options()("fitMethod",
      po::value<std::string>(&fitMethod)->default_value("plotOnly"),
      "choose between 'tree', 'amplitude' and 'plotOnly'");
  config_fit.add_options()("useMinos", po::value<bool>(&useMinos)->default_value(0),
      "Run MINOS for each parameter");
  config_fit.add_options()("useHesse", po::value<bool>(&useHesse)->default_value(1),
  ...
  LOG(info) << " create/modify minuitstragety.xml to change the Minuit2 stragety in minimization";
  ...
  if (fitMethod != "tree" && fitMethod != "amplitude" && fitMethod != "plotOnly") {
  ...
  //---------------------------------------------------
  auto fitModelKin = 
      std::make_shared<HelicityKinematics>(partList, fitModelTree.get_child("HelicityKinematics")); 
  ...
  //but one must create Intensity otherwise the HelicityKinematics object has no subsystems
  ...
  // Read Phase space sample which is used in PWA fit for xs section normalization
  ...
  //
  intens->setPhspSample(phspPoints, toyPoints); //if only fit, this Set is not necessary. If calculate fitfraction, this set is necessary
  ...
  if (dataFile != "") {
    ...
    int numSignalEvents = numEvents; //445 data and 10000 mc for wrong matched signal
