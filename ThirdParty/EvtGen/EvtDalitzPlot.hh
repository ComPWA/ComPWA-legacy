...
#include <assert.h>
// #include "EvtGen/EvtDecayMode.hh"

class EvtDalitzPlot {
public:
  EvtDalitzPlot();
  EvtDalitzPlot(double mA, double mB, double mC, double bigM, double ldel = 0.,
                double rdel = 0.);
  //   EvtDalitzPlot(const EvtDecayMode& mode, double ldel = 0., double rdel =
  //   0.);
  ...
  // Find the area of the Dalitz plot by numeric integration. (N bins for
  // variable q(i) are used). Very large numbers of N can result in a very long
  // calculation. It should not matter which two pairs f variables are used. The
  // integral should eventually converge to the same number
