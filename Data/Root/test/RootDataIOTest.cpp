...
void createRootTree(const std::string &OutputFileName,
  ...
  TTree TheChain(TreeName.c_str(), TreeName.c_str());
  TClonesArray parts("TParticle", 3);
  double weight;
  TheChain.Branch("Particles", &parts);
  TheChain.Branch("weight", &weight);
...
std::vector<ComPWA::Event> generateSample(double InitialStateMass,
  ...
  RootGenerator gen(InitialStateMass, FinalState);
...
BOOST_AUTO_TEST_CASE(SimpleReadWriteCheck) {
  ComPWA::Logging log("TRACE");

  using namespace ComPWA::Data::Root;
  auto SampleOut = generateSample(1.864, {0.5, 0.5, 0.5});
  const char *OutputFileName = "RootReaderTest-sameFile.root";
  writeData(SampleOut, OutputFileName, "TestTree");

  auto SampleIn(readData(OutputFileName, "TestTree"));
