...
void FunctionTree::insertTree(std::shared_ptr<FunctionTree> inTree, std::string parent){
	//	BOOST_LOG_TRIVIAL(debug)<<"FunctionTree::insertTree() | inserting tree: "
	//			<<inTree->head()->getName()<<" with "<<inTree->GetNumberOfNodes()<<" child nodes.";
	_childTrees.push_back(inTree);
	insertNode(inTree->head(),parent);
	return;
}
void FunctionTree::insertNode(std::shared_ptr<TreeNode> inNode, std::string parent){
	std::string n = inNode->getName();
	bool ret = nodes_.insert(
			std::pair<std::string, std::shared_ptr<TreeNode> >(inNode->getName(),inNode)
	).second;
	//	BOOST_LOG_TRIVIAL(debug)<<"FunctionTree::insertNode() | inserting node: "
	//			<<inNode->getName()<<" with "<<inNode->getChildren().size()<<" child nodes. OK? "<<ret;
...
void FunctionTree::addChildNodes(std::shared_ptr<TreeNode> startNode){
	...
	for(unsigned int i=0; i<newChildren.size(); i++){
		bool ret = nodes_.insert(std::pair<std::string, std::shared_ptr<TreeNode> >(
				newChildren.at(i)->getName(),newChildren.at(i))).second;
		//if(!ret)
		//	BOOST_LOG_TRIVIAL(debug)<<"FunctionTree::addChildNodes() | Failed to insert element "
		//	<<newChildren.at(i)->getName()<<" into list of nodes!";
		//else
		//	BOOST_LOG_TRIVIAL(debug)<<"FunctionTree::addChildNodes() | Inserting element "
		//	<<newChildren.at(i)->getName()<<" into list of nodes!";
...
void FunctionTree::getNamesDownward(std::shared_ptr<TreeNode> start,
		std::vector<std::string>& childNames, std::vector<std::string>& parentNames){
...
void FunctionTree::createHead(const std::string& name, std::shared_ptr<Strategy> strat,
		unsigned int dim, bool useVec){
	//	BOOST_LOG_TRIVIAL(debug)<<"FunctionTree::createHead() | creating new head: "<<name;
...
void FunctionTree::createNode(const std::string& name, std::shared_ptr<Strategy> strat,
		std::string parent, unsigned int dim, bool useVec){
	...
	if(dim==1 && !useVec)
		newNode = std::shared_ptr<TreeNode>(new TreeNode(name, inter[0], strat, parentNode));
	else
		newNode= std::shared_ptr<TreeNode>(new TreeNode(name, inter, strat, parentNode));

	nodes_.insert(std::pair<std::string, std::shared_ptr<TreeNode> >(name,newNode));
	newNode->linkParents();
	if(parent=="") head_ = newNode; //if we created a head redirect pointer
}
void FunctionTree::createLeaf(const std::string name, const double extPar, std::string parent){
	if(parent=="" && head_)
		throw std::runtime_error("FunctionTree::createNode() | head node already exists!");
...
bool FunctionTree::sanityCheck(){
	bool isSane=true;
	if(!head_)
		throw std::runtime_error("FunctionTree::sanityCheck() | This tree has no head!");
