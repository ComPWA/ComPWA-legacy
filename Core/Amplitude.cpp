...
void Amplitude::FillAmpParameterToList(
    std::vector<std::shared_ptr<Amplitude>> ampVec, ParameterList &list) {
...
double Amplitude::intensityInterference(dataPoint &point, resonanceItr A,
                                        resonanceItr B) {
  double intens =
      ((*A)->Evaluate(point) * std::conj((*B)->Evaluate(point))).real();
  if (A != B)
    intens = 2 * intens;
...
void Amplitude::SetAmpEfficiency(std::vector<std::shared_ptr<Amplitude>> ampVec,
...
void Amplitude::GetAmpFitFractions(
    std::vector<std::shared_ptr<Amplitude>> ampVec, ParameterList &parList) {
  if (!ampVec.size())
