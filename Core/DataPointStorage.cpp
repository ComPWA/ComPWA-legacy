...
unsigned int
DataPointStorage::getNumberOfEvents(unsigned int storage_index) const {
  return number_of_events_.at(storage_index);
}

void DataPointStorage::layoutDataStorageStructure(
    unsigned int storage_index, unsigned int expected_number_of_events,
    const Event &evt) {
...
const DataList &
DataPointStorage::getDataList(unsigned int storage_index,
