...
public:
  ...
  virtual ~FunctionTree() {
    std::map<std::string, std::shared_ptr<TreeNode>>::iterator iter =
        nodes_.begin();
  ...
  //! Create head node of FcnTree
  /*!
   * Create head node as a leaf. Constant parameter! We use this to generate an
   * empty tree.
   * \param name identifier of node
...
*\param useVec true if value is a vector
   ...
   */
  virtual void createLeaf(const std::string name,
                          std::vector<std::shared_ptr<AbsParameter>> &extPar,
                          std::string parent);
  ...
  //! trigger calculation
  void recalculate() { head_->recalculate(); }
...
protected:
  ...
  std::shared_ptr<TreeNode>
      head_; /*!< the head node storing the absolute result */
  std::map<std::string, std::shared_ptr<TreeNode>>
      nodes_; /*!< map to store the nodes */
