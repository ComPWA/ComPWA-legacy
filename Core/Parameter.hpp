...
public:
  ...
  DoubleParameter(std::string inName = "") :
      AbsParameter(inName, ParType::DOUBLE), fixed_(0), val_(0), min_(0), max_(
          0), errorType(ErrorType::NOTDEF) {
  ...
  DoubleParameter(std::string inName, const double value) :
      AbsParameter(inName, ParType::DOUBLE), fixed_(0), val_(value), min_(0), max_(
          0), errorType(ErrorType::NOTDEF) {
    bounds_ = usebounds_ = false;
  }
  ...
  //! Check if parameter has bounds
  virtual inline bool HasBounds() const {
    return bounds_;
  }
  //! Check if bounds should be used
  ...
  //! Check if parameter is fixed
  virtual inline bool IsFixed() const {
    return fixed_;
  }
  //! Set if bounds should be used
  ...
  //! Getter for lower bound of parameter
  virtual inline double GetMinValue() const {
    return min_;
  }
  //! Getter for upper bound of parameter
  virtual inline double GetMaxValue() const {
    return max_;
  }
  //! Getter for FunctionTree support
  ...
  virtual void SetValue(const double inVal) {
    if (fixed_)
      throw ParameterFixed(
          "DoubleParameter::SetValue() | Parameter " + GetName()
              + " is fixed!");
  ...
  virtual void SetMinMax(const double min, const double max) {
    ...
    if (!bounds_) {
      ...
      throw std::runtime_error(
          "DoubleParameter::SetMinMaxValue() bounds not valid: ["
...
protected:
  ...
  virtual void SetErrorHigh(double errHigh) {
    errorHigh = errHigh;
  }
  //! Setter for lower error of parameter
  virtual void SetErrorLow(double errLow) {
    errorLow = std::abs(errLow);
  }
