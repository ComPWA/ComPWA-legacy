...
public:
  ...
  virtual void setAmplitude(std::shared_ptr<Amplitude> amp,
                            ...
                            unsigned int nEvents = 0, bool useFuncTr = 0);
   ...
   * @param accSample_ sample of efficiency applied phsp events for
   * unbinned efficiency correction
   ...
   * A penalty term can be added to the LH to punish unnecessary complexity of
   * amplitudes in
   * minimization. The strength of this term can be set via
   * MinLogLH::setPenaltyScale(double).
