...
void EvtGenIF::addResonance(std::string name, double m0, double g0, double spin, ComPWA::SubSystem subsys){
  //LOG(debug) << "EvtGenIF::addResonance starts. Name: " << name << " mass: " << m0 << " width: " << g0 << " spin: " << spin;
  EvtCyclic3::Pair pairAng;
  EvtCyclic3::Pair pairRes;
  LOG(DEBUG) << "EvtGenIF::addResonance num finalstate00: "<< subsys.getFinalStates().at(0).at(0);
  LOG(DEBUG) << "EvtGenIF::addResonance num finalstate01: "<< subsys.getFinalStates().at(1).at(0);
  switch(subsys.getFinalStates().at(0).at(0)+subsys.getFinalStates().at(1).at(0)){
    case 3://12:
  ...
  EvtSpinType::spintype evtspin;
  if(spin == 0) evtspin = EvtSpinType::spintype::SCALAR;
  else if(spin == 1) evtspin = EvtSpinType::spintype::VECTOR;
  else if(spin == 2) evtspin = EvtSpinType::spintype::TENSOR;
  ...
  EvtDalitzReso reso(name, DalitzPlot, pairAng, pairRes,
		  evtspin, m0, g0, EvtDalitzReso::NumType::RBW_ZEMACH);
  Resos.push_back(reso);

  evtPars[std::string(name+"_mass")] = std::shared_ptr<ComPWA::FitParameter>(new ComPWA::FitParameter(name+"_mass", m0));
  evtPars.at(std::string(name+"_mass"))->fixParameter(false);
  evtPars[std::string(name+"_width")] = std::shared_ptr<ComPWA::FitParameter>(new ComPWA::FitParameter(name+"_width", g0));
...
void EvtGenIF::addHeliResonance(boost::property_tree::ptree pt, std::shared_ptr<PartList> partL){
  ...
  if (SubSys.getFinalStates().size() == 2) {
   ...
   //     J, mu, DecayHelicities.first - DecayHelicities.second);
    ...
    if (decayType == "stable") {
      throw std::runtime_error("EvtGenIF::addHeliResonance() | Stable particle is "
...
void EvtGenIF::addResonances(boost::property_tree::ptree pt, std::shared_ptr<DalitzKinematics> kin, std::shared_ptr<PartList> partL){
    ...
    if (v.first == "Intensity" &&
    	  ...
    	  for (const auto &w : v.second.get_child("")) {
    	    if (w.first == "Amplitude" &&
    	      ...
    	      //addAmplitude(std::make_shared<SequentialPartialAmplitude>(partL, kin, v.second));
    	    	  ...
    	    	  for (const auto &x : w.second.get_child("")) {
    	    	    if (x.first == "Parameter"){
    	    	      if (x.second.get<std::string>("<xmlattr>.Type") == "Magnitude")
    	    	        double magnitude = (std::make_shared<FitParameter>(x.second))->value();
    	    	      if (x.second.get<std::string>("<xmlattr>.Type") == "Phase")
    	    	        double phase = (std::make_shared<FitParameter>(x.second))->value();
    	    	    ...
    	    	    //               "NonResonant") {
    	    	    //  addPartialAmplitude(
    	    	    //      std::make_shared<ComPWA::Physics::NonResonant>(partL, kin, x.second));
...
EvtGenIF::component(std::string name) {
