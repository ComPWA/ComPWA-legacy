...
NonResonant::NonResonant(const char *name, std::shared_ptr<DoubleParameter> mag,
    ...
    : AmpAbsDynamicalFunction(
          name, 0, 0, mag, phase,
          std::make_shared<DoubleParameter>("mass", 0.0), ComPWA::Spin(0.0),
          ComPWA::Spin(0.0), ComPWA::Spin(0.0), +1, 0, mother, particleA,
          particleB, formFactorType::noFormFactor, nCalls, nS) {}
...
std::shared_ptr<FunctionTree> NonResonant::GetTree(ParameterList &sample,
  ...
  if (_normStyle == normStyle::none) {
    ...
    //);
    //		newTree->createLeaf("NormNonRes_"+_name, unitVec2,
    //"AbsVal_"+_name);
  }
  return newTree;
}
