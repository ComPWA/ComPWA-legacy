...
public:
  WignerDStrategy(const std::string resonanceName, ParType in) :
      Strategy(in), name(resonanceName) {
    //name = +resonanceName;
  }
  ...
  virtual bool execute(ParameterList& paras,
    ...
    if (checkType != out->type()) {
      throw(WrongParType(
          std::string("Output Type ") + ParNames[out->type()]
              + std::string(" conflicts expected type ") + ParNames[checkType]
              + std::string(" of ") + name + " Wigner strat"));
      return false;
    }
    ...
    unsigned int _subSysFlag = (unsigned int) (paras.GetParameterValue(
        "ParOfNode_subSysFlag_" + name));
