...
public:
  ...
  AmpSumSliceIntensity(const double inM, const double inBr, const double in1,
                       const double in2, const double in3, AmplitudeSetup ini)
      : M(inM), Br(inBr), m1(in1), m2(in2), m3(in3), PI(3.14159),
        m23_sq_min((m2 + m3) * (m2 + m3)), m23_sq_max((M - m1) * (M - m1)),
        m13_sq_min((m1 + m3) * (m1 + m3)), m13_sq_max((M - m2) * (M - m2)),
        m12_sq_min((m1 + m2) * (m1 + m2)), m12_sq_max((M - m3) * (M - m3)),
        m23_min((m2 + m3)), m23_max((M - m1)), m13_min((m1 + m3)),
        m13_max((M - m2)), m12_min((m1 + m2)), m12_max((M - m3)),
        ma("ma", "mass", m23_min, m23_max), mb("mb", "mass", m13_min, m13_max),
    ...
    for (std::vector<Resonance>::iterator reso = ini.getResonances().begin();
      ...
      Resonance tmp = (*reso);
      // setup RooVars
      mr.push_back(std::shared_ptr<RooRealVar>(
          new RooRealVar(("m_{" + tmp.m_name + "}").c_str(), "resMass",
                         tmp.m_mass, tmp.m_mass_min, tmp.m_mass_max)));
      qr.push_back(std::shared_ptr<RooRealVar>(
          new RooRealVar(("q_{" + tmp.m_name + "}").c_str(), "breakupMom",
                         tmp.m_breakup_mom)));
      gr.push_back(std::shared_ptr<RooRealVar>(
          new RooRealVar(("g_{" + tmp.m_name + "}").c_str(), "resWidth",
                         tmp.m_width, tmp.m_width_min, tmp.m_width_max)));
      rr.push_back(std::shared_ptr<RooRealVar>(new RooRealVar(
          ("r_{" + tmp.m_name + "}").c_str(), "amplitude", tmp.m_strength)));
      phir.push_back(std::shared_ptr<RooRealVar>(new RooRealVar(
          ("phi_{" + tmp.m_name + "}").c_str(), "phase", tmp.m_phase)));

      aj.push_back(std::shared_ptr<RooRealVar>(
          new RooRealVar(("j_{" + tmp.m_name + "}").c_str(), "j", tmp.m_spin)));
      am.push_back(std::shared_ptr<RooRealVar>(
          new RooRealVar(("m_{" + tmp.m_name + "}").c_str(), "m", tmp.m_m)));
      an.push_back(std::shared_ptr<RooRealVar>(
          new RooRealVar(("n_{" + tmp.m_name + "}").c_str(), "n", tmp.m_n)));

      // setup Dynamics and Angular Distribution
      unsigned int last = mr.size() - 1;
      if (tmp.m_daugtherA == 2 && tmp.m_daugtherB == 3) {
        std::shared_ptr<AmpRelBreitWignerRes> tmpbw(new AmpRelBreitWignerRes(
            tmp.m_name.c_str(), tmp.m_name.c_str(), ma, *mr[last], *gr[last],
            *qr[last], 1, tmp.m_spin));
        tmpbw->setDecayMasses(m2, m3);
        rbw.push_back(tmpbw);
        angd.push_back(std::shared_ptr<AmpWigner>(
            new AmpWigner(("a_{" + tmp.m_name + "}").c_str(),
                          ("a_{" + tmp.m_name + "}").c_str(), mc, ma, mb, 1,
                          *aj[last], *am[last], *an[last])));
        totAmp.addBW(rbw.at(last), rr.at(last), phir.at(last), angd.at(last));
      } else if (tmp.m_daugtherA == 1 && tmp.m_daugtherB == 3) {
        std::shared_ptr<AmpRelBreitWignerRes> tmpbw(new AmpRelBreitWignerRes(
            tmp.m_name.c_str(), tmp.m_name.c_str(), mb, *mr[last], *gr[last],
            *qr[last], 2, tmp.m_spin));
        tmpbw->setDecayMasses(m1, m3);
        rbw.push_back(tmpbw);
        angd.push_back(std::shared_ptr<AmpWigner>(
            new AmpWigner(("a_{" + tmp.m_name + "}").c_str(),
                          ("a_{" + tmp.m_name + "}").c_str(), mc, ma, mb, 2,
                          *aj[last], *am[last], *an[last])));
        totAmp.addBW(rbw.at(last), rr.at(last), phir.at(last), angd.at(last));
      } else if (tmp.m_daugtherA == 1 && tmp.m_daugtherB == 2) {
        std::shared_ptr<AmpRelBreitWignerRes> tmpbw(new AmpRelBreitWignerRes(
            tmp.m_name.c_str(), tmp.m_name.c_str(), mc, *mr[last], *gr[last],
            *qr[last], 3, tmp.m_spin));
        tmpbw->setDecayMasses(m1, m2);
        rbw.push_back(tmpbw);
        angd.push_back(std::shared_ptr<AmpWigner>(
            new AmpWigner(("a_{" + tmp.m_name + "}").c_str(),
                          ("a_{" + tmp.m_name + "}").c_str(), mc, ma, mb, 3,
                          *aj[last], *am[last], *an[last])));
        totAmp.addBW(rbw.at(last), rr.at(last), phir.at(last), angd.at(last));
  ...
  virtual const double intensity(std::vector<double> &x, ParameterList &par) {
    ...
    if (par.GetNDouble() > rr.size()) {
      std::cout << "Error: Parameterlist doesn't match model!!"
    ...
    RooComplex results[2];
    double AMPpdf = totAmp.evaluateSlice((results, 5); //TODO

    if(AMPpdf!=AMPpdf){
      // std::cout << "Error: Intensity is not a number!!" << std::endl; //TODO:
      // exception
      return 0;
    }
  ...
  virtual const bool fillStartParVec(ParameterList &outPar) {
    ...
    for (unsigned int i = 0; i < rr.size(); i++) {
      ...
      if (rr[i]->hasError()) // TODO: check bounds
        outPar.AddParameter(DoubleParameter(rr[i]->getVal(), rr[i]->getMin(),
                                            rr[i]->getMax(),
                                            rr[i]->getError()));
      else
        outPar.AddParameter(DoubleParameter(rr[i]->getVal(), 0.1));
      // outPar.AddParameter(DoubleParameter(phir[i]->getVal(),
      // phir[i]->getMin(), phir[i]->getMax(), phir[i]->getError()));
    }
    // outPar.AddParameter(DoubleParameter(rr[rr.size()-1]->getVal(),
    // rr[rr.size()-1]->getMin(), rr[rr.size()-1]->getMax(),
    // rr[rr.size()-1]->getError()));
...
protected:
    ...
    return m1 * m1 + m3 * m3 +
           0.5 / m23_sq *
               ((M * M - m23_sq - m1 * m1) * (m23_sq - m2 * m2 + m3 * m3) +
                sqrt(lambda(m23_sq, M * M, m1 * m1)) *
                    sqrt(lambda(m23_sq, m2 * m2, m3 * m3)));
  }
  ...
  double m23_sq_max_constr(double &m13_sq) {
    return m2 * m2 + m3 * m3 +
           0.5 / m13_sq *
               ((M * M - m13_sq - m2 * m2) * (m13_sq - m1 * m1 + m3 * m3) +
                sqrt(lambda(m13_sq, M * M, m2 * m2)) *
                    sqrt(lambda(m13_sq, m1 * m1, m3 * m3)));
  }
  ...
  double mb_sq_max_constr(double &ma_sq) { return m13_sq_max_constr(ma_sq); }

  double mb_sq_min_constr(double &ma_sq) { return m13_sq_min_constr(ma_sq); }

  double ma_sq_max_constr(double &mb_sq) { return m23_sq_max_constr(mb_sq); }

  double ma_sq_min_constr(double &mb_sq) { return m23_sq_min_constr(mb_sq); }
