...
AmpRelBreitWignerRes::AmpRelBreitWignerRes(
    ...
    std::shared_ptr<DoubleParameter> motherRadius, formFactorType type,
...
std::complex<double> AmpRelBreitWignerRes::EvaluateAmp(dataPoint &point) {
  ...
  try {
    result = dynamicalFunction(mSq, _mass->GetValue(), _mass1, _mass2,
...
AmpRelBreitWignerRes::SetupTree(ParameterList &sample, ParameterList &toySample,
  ...
  //----Add Nodes
  std::shared_ptr<BreitWignerStrategy> rbwStrat(new BreitWignerStrategy(_name));
