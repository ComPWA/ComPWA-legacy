    ...
    namespace HelicityFormalism {
      ...
      public:
        ...
        bool operator ==(const SubSystem &b) const{
          if( _recoilState == b._recoilState && _finalStateA == b._finalStateA && _finalStateB == b._finalStateB)
            return true;
          return false;
        }
        ...
        friend std::ostream& operator<< (std::ostream& stream, const SubSystem& s) {
          ...
          for( auto i : s._recoilState ) stream <<i<<" ";
          stream << ") finalParticlesA -> (";
          for( auto i : s._finalStateA ) stream <<i<<" ";
          stream << ") finalParticlesB -> (";
          for( auto i : s._finalStateB ) stream <<i<<" ";
          stream << ")";
          return stream;
        }
