...
DalitzKinematics::DalitzKinematics(std::string _nameMother, std::string _name1,
    ...
    Br(0.0), nameMother(_nameMother), name1(_name1), name2(_name2), name3(
        _name3), massIdsSet(false) {
...
DalitzKinematics::DalitzKinematics(double _M, double _Br, double _m1,
    double _m2, double _m3, std::string _nameMother, std::string _name1,
    std::string _name2, std::string _name3) :
    M(_M), Br(_Br), m1(_m1), m2(_m2), m3(_m3), nameMother(_nameMother), name1(
        _name1), name2(_name2), name3(_name3), massIdsSet(false) {
...
void DalitzKinematics::translateEventToDataPoint(const Event& ev,
...
double DalitzKinematics::invMassMax(unsigned int sys, unsigned int sys2,
  ...
  double max = (Efirst + Esecond) * (Efirst + Esecond)
      - (sqrt(Efirst * Efirst - Mfirst * Mfirst)
          - sqrt(Esecond * Esecond - Msecond * Msecond))
          * (sqrt(Efirst * Efirst - Mfirst * Mfirst)
              - sqrt(Esecond * Esecond - Msecond * Msecond));
  return max;
}
double DalitzKinematics::invMassMin(unsigned int sys, unsigned int sys2,
    double invMass_sys) const {
  unsigned int part1, part2;
  double Mfirst, Msecond;
  switch (sys) {
  ...
  double min = (Efirst + Esecond) * (Efirst + Esecond)
      - (sqrt(Efirst * Efirst - Mfirst * Mfirst)
          + sqrt(Esecond * Esecond - Msecond * Msecond))
          * (sqrt(Efirst * Efirst - Mfirst * Mfirst)
              + sqrt(Esecond * Esecond - Msecond * Msecond));
...
double DalitzKinematics::eiCms(unsigned int partId, unsigned int sys,
    double invMass_sys) const {
  double E;
  ...
  case 3:
    switch (partId) {
    case 1:
      E = (invMass_sys - mSq2 + mSq1) / (2 * m);
      break;
    case 2:
      E = (invMass_sys - mSq1 + mSq2) / (2 * m);
      break;
    case 3:
      E = -(invMass_sys - Msq + mSq3) / (2 * m);
      break;
    }
    break;
  case 4:
    switch (partId) {
    case 1:
      E = (invMass_sys - mSq3 + mSq1) / (2 * m);
      break;
    case 2:
      E = -(invMass_sys - Msq + mSq2) / (2 * m);
      break;
    case 3:
      E = (invMass_sys - mSq1 + mSq3) / (2 * m);
      break;
    }
    break;
  case 5:
    switch (partId) {
    case 1:
      E = -(invMass_sys - Msq + mSq1) / (2 * m);
      break;
    case 2:
      E = (invMass_sys - mSq3 + mSq2) / (2 * m);
      break;
    case 3:
      E = (invMass_sys - mSq2 + mSq3) / (2 * m);
      break;
    }
    break;
  }
...
double DalitzKinematics::scatteringAngle(double s, double t, double M,
    double mSpec, double mSecond, double m) {
  double u = getThirdVariableSq(s, t);
  double qSq = (s - (M + mSpec) * (M + mSpec)) * (s - (M - mSpec) * (M - mSpec))
      / (4 * s);
  double qPrimeSq = (s - (mSecond + m) * (mSecond + m))
      * (s - (mSecond - m) * (mSecond - m)) / (4 * s);
  double cosAngle = (s * (t - u)
      + (M * M - mSpec * mSpec) * (mSecond * mSecond - m * m))
...
double DalitzKinematics::helicityAngle(unsigned int sys, double invMassSq23,
  ...
  case 3:    //angle versus particle 2
    m = m2;
    mSpec = m3;
    invMsqSys = invMassSq12;
    invMsqSecond = invMassSq23;
  ...
  case 4:    //angle versus particle 1
    m = m1;
    mSpec = m2;
    invMsqSys = invMassSq13;
    invMsqSecond = invMassSq12;
...
//		eCms = eiCms(3,sys,invMsqSys);
  ...
  cosAngle = -(invMsqSecond - m * m - mSpec * mSpec - 2 * eCms * eSpecCms)
      / (2 * pCms * pSpecCms);
...
bool DalitzKinematics::isWithinPhsp(const dataPoint& point) {
  ...
  bool c1 = 0;
  bool c2 = 0;
  if (m13sq >= invMassMin(4, 5, m23sq) && m13sq <= invMassMax(4, 5, m23sq))
    c0 = 1;
  if (m12sq >= invMassMin(3, 5, m23sq) && m12sq <= invMassMax(3, 5, m23sq))
    c1 = 1;
  if (m23sq >= invMassMin(5, 4, m13sq) && m23sq <= invMassMax(5, 4, m13sq))
    c2 = 1;
  if (c0 && c1 && c2)
    return 1;
  //if(c0 && c1) return 1;
