 ...
 * 			DalitzKinematics stores particle masses of a process and
 *provides some functions which calculate kinematic
 * 			quantities.
 */
...
public:
  static Kinematics *createInstance(std::string nameMother, std::string name1,
   ...
   *
   * Function obsolete!
   * Calculates the scattering angle given the invariant masses @param s and
   * @param t .
   * The angle is measured between the spectator particle @param mSpec and
   * particle @param m .
   ...
   * When masses for scatteringAngle() are set correctly both functions are
   * equivalent.
  ...
  //! Returns the dalitz plot area for the given kinematics and limited m23
  //! range
  double getPhspVolumePart(double, double);

  //! Calculated momenta n,m using legendre polynomials
  double calculateMoments(unsigned int sys, dataPoint &point, unsigned int n,
  ...
  //! Energy of particle partId in rest frame of system sys at the invariant
  //! mass invMass_sys
...
protected:
  ...
  //! constructor access particles by name, masses etc are obtained from
  //! PhysConst singleton
