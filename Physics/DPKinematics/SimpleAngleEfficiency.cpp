...
// effHist(new SimpleEfficiency(*eff)){
SimpleAngleEfficiency::SimpleAngleEfficiency(SimpleEfficiency *eff) {
  effHist = std::shared_ptr<SimpleEfficiency>(new SimpleEfficiency(*eff));
  LOG(debug) << "SimpleAngleEfficiency: creating efficiency from existing "
                "SimpleEfficiency object!";
}
SimpleAngleEfficiency::SimpleAngleEfficiency(TH1 *passed, TH1 *total)
    : effHist(new SimpleEfficiency("angleEff", "angleEff", *passed, *total)) {
  LOG(debug) << "SimpleAngleEfficiency: creating efficiency from two "
                "histograms objects!";
}
SimpleAngleEfficiency::SimpleAngleEfficiency(
    const SimpleAngleEfficiency &other) {
...
double SimpleAngleEfficiency::evaluate(const dataPoint &point) {
  ...
  //	std::cout<<globalBin<<" m="<<m23sq<<" theta="<<angle<<"
  //->"<<eff<<std::endl;
  return eff;
}
