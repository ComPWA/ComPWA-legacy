...
std::complex<double> Kinematics::qValue(double sqrtS, double ma, double mb) {
  ...
  if (result.real() != result.real() || result.imag() != result.imag()) {
    std::cout << "AmpKinematics::qValue() | NaN! sqrtS=" << sqrtS << " ma="
        << ma << " mb=" << mb << std::endl;
  }
  return result;
}
...
double Kinematics::FormFactor(double sqrtS, double ma, double mb, double spin,
 ...
 // std::cout << qSqValue(sqrtS, ma, mb) << " " << mesonRadius <<std::endl;
  ...
  if (spin == 1) {
    ...
    std::cout << "Wrong spin value! BLW factors only implemented for spin 0-4! "
        << std::endl;
  }
  return 0;
}
...
//  std::cout << " haeh? " << s << " " << ma << " " << mb << " "
//      << (ma + mb) * (ma + mb) << std::endl;
  ...
  if (s <= 0) {    //below 0
    ...
    rho = (-q / M_PI * log(abs((1 + q) / (1 - q))) + i * q)
        / (i * 16.0 * M_PI * sqrtS);
  }
  else
    throw std::runtime_error(
        "AmpKinematics::phspFactor| phspFactor not defined at sqrtS = "
            + std::to_string((long double) sqrtS));

  if (rho.real() != rho.real() || rho.imag() != rho.imag()) {
    std::cout << "AmpKinematics::phspFactor() | NaN! sqrtS=" << sqrtS << " ma="
        << ma << " mb=" << mb << std::endl;
  }
  return rho;    //correct analytical continuation
}

std::complex<double> Kinematics::widthToCoupling(double mSq, double mR,
  ...
  std::complex<double> result = std::complex<double>(sqrt(mR * width), 0)
      / denom;
  if(std::abs(result) > 1e8) {
    std::cout<<rho<<" "<<denom<<" "<<ffR<<std::endl;
  }
  return result;
}

std::complex<double> Kinematics::couplingToWidth(double mSq, double mR,
    double g, double ma, double mb, double spin, double mesonRadius) {
  ...
  if (result.real() != result.real() || result.imag() != result.imag()) {
    std::cout << "AmpKinematics::couplingToWidth() | NaN! mSq=" << mSq << " mR="
        << mR << " g=" << g << " ma=" << ma << " mb=" << mb << std::endl;
