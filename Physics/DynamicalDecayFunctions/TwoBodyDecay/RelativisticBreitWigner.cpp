...
RelativisticBreitWigner::evaluate(const dataPoint &point) const {
  double mSq = point.getVal(dataIndex);
  double result =  dynamicalFunction(mSq, _mass->GetValue(), _mass1, _mass2,
                           _width->GetValue(), (double)_spin, _mesonRadius->GetValue(),
                           _ffType);
